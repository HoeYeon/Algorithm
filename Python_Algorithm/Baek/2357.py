...
def make_max_tree(idx,start,end):
    if start == end:
        max_tree[idx] = arr[start]
    else:
        max_tree[idx] = max(make_max_tree(idx*2,start,(start+end)//2),make_max_tree(idx*2+1,(start+end)//2+1,end))
    return max_tree[idx]
...
def get_max(idx,wl,wr,start,end):
    if wr < start or wl > end:
        return -1
    elif wl <= start and wr >= end:
        return max_tree[idx]
    else:
        mmax = max(get_max(idx*2,wl,wr,start,(start+end)//2),get_max(idx*2+1,wl,wr,(start+end)//2+1,end))
        return mmax
...
max_tree = [0 for i in range(tree_size)]
make_min_tree(1,0,N-1)
make_max_tree(1,0,N-1)
result = ''
for i in range(M):
    a,b = map(int,input().split())
    result += str(get_min(1,a-1,b-1,0,N-1)) + ' ' + str(get_max(1,a-1,b-1,0,N-1))+'\n'
